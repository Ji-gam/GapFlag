...
v1.3 · 작성 2026-08-16, 갱신 2026-09-01 → 2026-09-04(v1.2) → 2026-09-04(v1.3) · 이 문서 하나만 읽으면 이전 대화 없이 이어서 작업할 수 있다.

**v1.3에서 바뀐 것**: v1.2(같은 날) 이후 **S0(소개·산식 공개 화면)이 [PR #22](https://github.com/Ji-gam/GapFlag/pull/22)로 머지**됐다 — §4-5의 마지막 미착수 P1 화면이 닫혔다. 이어서 `.serena/` `graphify-out/` 등 로컬 도구 산출물을 `.gitignore`에 추가하는 [PR #23](https://github.com/Ji-gam/GapFlag/pull/23)이 `dev`로 올라갔다(리뷰 대기). §0·§2-1·§4-5·§6·§7을 갱신했다. §1·§3·§4-1~4-4·§4-6·§5·§8·§10은 그대로 유효.
...
| 현재 단계 | **화면 4개 전부 동작함** (검색·매트릭스·상세·소개). DB 모델·수집 서비스 5종(R1/R2/R3/O1/O2)까지 실제로 켜서 확인 가능. 기회지수 커버리지 66.7%. R4(EPO)·O3 미수집, S3(비교)는 여전히 미착수(P1, 잘라도 됨), 시드 성분 34건은 초안(팀 검수 전) |
...
| MVP 범위 | **엔드포인트 3개 / 화면 3개 + 소개(S0)** | 늘리지 않는다 |
...
app/apis/cmp_pages.py              GET / , GET /matrix , GET /compound/{ingredient} , GET /about ✅ NEW
app/templates/                     base.html, search.html, matrix.html, detail.html, about.html  ✅ NEW
...
### 2-2. S0(소개·산식 공개) 화면 — 이번 세션 추가

- `GET /about` (`cmp_pages.py`) → `about.html` 정적 렌더링. DB 조회 없음(산식·출처는 고정 텍스트).
- 내용: 위험·기회 산식 전문, 구성요소별 환산식·가중치, 데이터 출처 표, R4/O3 미수집 사유,
  NULL≠0 원칙, 2×2 사분면 표. 발표에서 "왜 70점인가" 질문에 그 자리에서 답하는 유일한 화면.
- `search.html`·`matrix.html` 상단에 `/about` 링크 추가.
- **가중치를 화면에서 조정하는 UI는 범위 밖으로 뺐다** — "화면은 DB만 읽는다" 원칙과 충돌.
  CLAUDE.md가 요구하는 "사용자가 조정 가능해야" 하는 부분은 아직 실제 조정 기능이 아니라
  **산식 공개**까지만 구현된 상태 — 필요하면 다음 과제로 남겨둘 것.
- 렌더링 테스트 1건 추가(httpx ASGI client) → `app/tests/` **37개**로 증가, 전부 통과.
- **[PR #22](https://github.com/Ji-gam/GapFlag/pull/22)** `feature/about-page-s0` → `dev`, 2026-09-04 머지 완료.

### 2-3. 재수집 결과 (2026-09-04, `data/gapflag.db` 실측, O2 반영 기준)

- 35행(성분×종 조합) 전부 **기회지수 커버리지 66.7%**(O1+O2 / 전체 3개 중 2개)
...
DB에 저장된 **모든 성분·종 조합을 한 번에 산점도로** 그린다(`cmp_repository.list_scores` →
`cmp_service.list_matrix_points` → `matrix.html`의 SVG 루프). 특정 성분 하이라이트(빨간 원),
지수 NULL 조합은 매트릭스 아래 회색 목록으로 별도 표시("위험이 없다는 뜻이 아닙니다"). 점 클릭
→ 근거 카드 이동.

### 2-5. Alembic / 테스트

- Alembic 리비전 1개 존재(`f84742b34aad_add_compound_cmp_models.py`), 스키마 동결 중
- `app/tests/` 테스트 **37개**(v1.2의 36 + S0 렌더링 테스트 1), 전부 통과.
  `app/tests/fixtures/`에 저장된 API 응답으로 테스트 — 실 API 호출 없음

### 2-6. `.gitignore` 정리 (2026-09-04, 이번 세션)

- `.serena/`(Serena MCP 로컬 인덱스), `graphify-out/`(graphify 스킬 산출물),
  `KU_AI_Nest_팀_아이디어_비교표.pdf`(개인 로컬 참고 자료)를 `.gitignore`에 추가.
  전부 팀 공유용 산출물이 아니라 로컬 도구/개인 파일이라 커밋 대상에서 제외.
- **[PR #23](https://github.com/Ji-gam/GapFlag/pull/23)** `feature/about-page-s0`(gitignore 커밋만 추가) → `dev`, 리뷰 대기 중.
...
`http://localhost:8000/matrix` 에서 산점도가, `http://localhost:8000/about` 에서 산식 공개
화면이 보이면 완료. Docker는 이은호만, 배포 직전에.
...
기본 가중치 — 위험 R1~R4 각 25 / 기회 O1 40, O2 30, O3 30. **이 산식 전문은 이제 `/about`
화면에서 사용자가 직접 볼 수 있다**(§2-2). 계수(25·30·40)는 **초기 가정**이다. O2 추가 후
실측(35건, 2026-09-04): 위험지수 19.2~81.6, 기회지수 0.0~70.9로 상단까지 퍼짐. O3까지
채워지면 재분포될 가능성 있음. 4분면에 고르게 흩어지지 않으면 조정하고 이력을 남긴다.
...
app/services/src_opentargets_service.py ✅ / src_clinicaltrials_service.py ✅
...
app/apis/cmp_pages.py              ✅ HTML 페이지 (Jinja2) — /api/v1 아래 아님. /about 포함 4개 라우트
app/templates/                     ✅ base/search/matrix/detail/about.html
...
| S0 | 소개 · 데이터 출처 · **산식 전문 공개** | P1 | ✅ 구현됨 (2026-09-04, PR #22) |

**남은 화면은 S3 하나뿐** — §4-6 잘라내는 순서상 가장 먼저 버려도 되는 화면이라 시간이
부족하면 착수하지 않아도 발표에 지장 없다.
...
- `docs/REQUIREMENTS(요구사항정의서).md` — SRS v1.0 (2026-08-29). **R3/O2/S0 구현이 SRS 작성
  이후 들어갔으므로 문서가 코드보다 뒤처져 있음** — 실제로는 R1/R2/R3/O1/O2 5개 구현 + S0 완료
...
- `docs/SESSION_HANDOFF.md`(v1.0) → `docs/SESSION_HANDOFF_1.md`(v1.1) → `docs/SESSION_HANDOFF_2.md`(v1.2)
  → 이 문서(v1.3) — 세션 인수인계 스냅샷. 과거 버전도 지우지 말고 그대로 둔다(이력 추적용)
...
- `KU_AI_Nest_팀_아이디어_비교표.pdf` — 4인 아이디어 비교 (이번 세션부터 `.gitignore` 대상, §2-6)
...
3. **[PR #23](https://github.com/Ji-gam/GapFlag/pull/23) 머지** — `.gitignore` 정리만 있는
   가벼운 PR, 리뷰 후 바로 머지해도 됨
...
6. S3(후보 비교) 화면 — P1, 시간이 남을 때만
7. `docs/REQUIREMENTS(요구사항정의서).md` R3/O2/S0 반영 갱신 (발표엔 영향 없음, 문서 정합성용)
...
docs/SESSION_HANDOFF_3.md 를 먼저 읽어라. git log와 origin 대비 로컬 상태도 확인해라
...
- **이 핸드오프 문서 자체도 스냅샷이다.** 새 세션 시작 시 `git log`로 origin과 로컬을 대조하고,
  ...
  확인하는 습관을 들일 것.
- 프론트엔드는 없다 — `/matrix`, `/compound/{ingredient}`, `/about` 등은 FastAPI가 Jinja2로
  직접 그리는 화면이다. `frontend/`(React) 폴더의 dev 서버 에러는 무시해도 된다.
...
1. *"왜 70점인가?"* → 총점만 보여주지 않는다. 구성요소·가중치·산식을 전부 공개하고(`/about`), 사용자가 직접 근거를 추적할 수 있다.
